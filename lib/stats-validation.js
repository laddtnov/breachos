...
function sanitizeStats(incoming, stored) {
  ...
  const stats = {
    ...
    unlockedSkins: Array.isArray(incoming.unlockedSkins)
      ? incoming.unlockedSkins.slice(0, STAT_LIMITS.unlockedSkins)
          .map(s => cleanString(s)).filter(Boolean)
      : ['default'],
    unlockedAchievements: Array.isArray(incoming.unlockedAchievements)
      ? incoming.unlockedAchievements.slice(0, STAT_LIMITS.unlockedSkins)
          .map(s => cleanString(s)).filter(Boolean)
      : [],
  ...
  if (incoming.dailyBestTimes && typeof incoming.dailyBestTimes === 'object') {
    for (const key of Object.keys(incoming.dailyBestTimes).slice(0, STAT_LIMITS.gameHistory)) {
      if (!/^\d{4}-\d{2}-\d{2}$/.test(key)) continue;
      const n = Number(incoming.dailyBestTimes[key]);
      if (Number.isFinite(n) && n > 0) {
        stats.dailyBestTimes[key] = Math.min(Math.trunc(n), STAT_LIMITS.seconds);
      }
    }
  }
