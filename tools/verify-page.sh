...
# service worker disabled. Local verification only; _verify.html is gitignored.
#
...
s = re.sub(r'src="(js/[^"?]+)"', lambda m: f'src="{m.group(1)}?v={v}"', s)
s = re.sub(r'href="(css/[^"?]+)"', lambda m: f'href="{m.group(1)}?v={v}"', s)
